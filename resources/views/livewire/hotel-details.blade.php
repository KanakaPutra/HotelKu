<div class="bg-white rounded-lg shadow-xl overflow-hidden">
    ...
    {{-- ========== TAHAP 1: PEMILIHAN KAMAR ========== --}}
    @if ($step == 1)
        <div>
            <img class="w-full h-96 object-cover"
                 src="{{ $hotel->image_path ? asset('storage/' . $hotel->image_path) : 'https://placehold.co/1200x400/EBF4FF/7F9CF5?text=Hotel+Image' }}"
                 alt="Gambar {{ $hotel->name }}">
            <div class="p-8">
            ...
            <div class="space-y-6">
                @forelse($hotel->rooms as $room)
                    <div class="border rounded-lg p-4 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div class="flex-1 w-full md:w-auto">
                            ...
                            <p class="font-bold text-xl text-blue-600 mt-2">
                        ...
                        <div class="w-full md:w-auto">
                            <div class="flex flex-col sm:flex-row items-end space-y-2 sm:space-y-0 sm:space-x-2">
                                <div>
                                    ...
                                    <input type="date" id="checkin-{{ $room->id }}" wire:model.defer="checkInDate"
                                ...
                                <div>
                                    ...
                                    <input type="date" id="checkout-{{ $room->id }}" wire:model.defer="checkOutDate"
                                           required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm">
                                </div>

                                <button wire:click="bookRoom({{ $room->id }})" class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">
                                    Pesan
                                </button>
                ...
                @empty
                    <p class="text-center text-gray-500">
                        Saat ini belum ada kamar yang tersedia untuk hotel ini.
                    </p>
                @endforelse
            </div>
        </div>
            ...
            <div wire:loading.remove>
                ...
                <div class="mt-8 flex flex-col sm:flex-row-reverse sm:justify-between sm:items-center gap-4">
                    <button wire:click="confirmBooking"
                            ...
                            class="w-full sm:w-auto bg-blue-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-blue-700 text-lg transition-colors">
                    ...
                    <button wire:click="$set('step', 1)"
                            class="w-full sm:w-auto text-gray-600 font-medium hover:underline text-center sm:text-left">
